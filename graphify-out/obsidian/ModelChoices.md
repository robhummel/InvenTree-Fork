---
source_file: "/Users/Rob/Services/inventree-source/inventree/src/backend/InvenTree/report/models.py"
type: "code"
community: "Community 4"
location: "L197"
tags:
  - graphify/code
  - graphify/INFERRED
  - community/Community_4
---

# ModelChoices

## Connections
- [[ColorEnum]] - `uses` [INFERRED]
- [[DataImportStatusCode]] - `uses` [INFERRED]
- [[DataOutput]] - `uses` [INFERRED]
- [[InvenTreeAttachmentMixin]] - `uses` [INFERRED]
- [[InvenTreeMetadata]] - `uses` [INFERRED]
- [[InvenTreeParameterMixin]] - `uses` [INFERRED]
- [[InvenTreePlugin]] - `uses` [INFERRED]
- [[InvenTreeSettingsKeyType]] - `uses` [INFERRED]
- [[MetadataMixin_1]] - `uses` [INFERRED]
- [[Model choices for attachments.]] - `rationale_for` [EXTRACTED]
- [[Model choices for data import sessions.]] - `rationale_for` [EXTRACTED]
- [[Model choices for parameters.]] - `rationale_for` [EXTRACTED]
- [[Model choices for parameters._1]] - `rationale_for` [EXTRACTED]
- [[Model choices for report templates.]] - `rationale_for` [EXTRACTED]
- [[PluginMixinEnum]] - `uses` [INFERRED]
- [[RenderChoices]] - `inherits` [EXTRACTED]
- [[RenderChoices_1]] - `uses` [INFERRED]
- [[SettingsKeyType_2]] - `uses` [INFERRED]
- [[StatusCode_1]] - `uses` [INFERRED]
- [[StringEnum_1]] - `uses` [INFERRED]
- [[UpdatedUserMixin]] - `uses` [INFERRED]
- [[models.py_1]] - `contains` [EXTRACTED]
- [[models.py_3]] - `contains` [EXTRACTED]
- [[models.py_6]] - `contains` [EXTRACTED]

#graphify/code #graphify/INFERRED #community/Community_4