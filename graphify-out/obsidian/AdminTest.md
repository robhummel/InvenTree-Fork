---
source_file: "/Users/Rob/Services/inventree-source/inventree/src/backend/InvenTree/stock/tests.py"
type: "code"
community: "Core Models & Admin"
location: "L1588"
tags:
  - graphify/code
  - graphify/INFERRED
  - community/Core_Models_&_Admin
---

# AdminTest

## Connections
- [[.test_admin()]] - `method` [EXTRACTED]
- [[AdminTestCase]] - `inherits` [EXTRACTED]
- [[AdminTestCase_1]] - `uses` [INFERRED]
- [[ApiToken]] - `uses` [INFERRED]
- [[Attachment]] - `uses` [INFERRED]
- [[Company]] - `uses` [INFERRED]
- [[CustomUnit]] - `uses` [INFERRED]
- [[DataImportRow]] - `uses` [INFERRED]
- [[DataImportSession]] - `uses` [INFERRED]
- [[GlobalWarningCode]] - `uses` [INFERRED]
- [[ImporterMixin]] - `inherits` [EXTRACTED]
- [[InvenTreeAPITestCase_1]] - `uses` [INFERRED]
- [[InvenTreeCustomUserStateModel]] - `uses` [INFERRED]
- [[InvenTreeSetting]] - `uses` [INFERRED]
- [[InvenTreeTestCase_1]] - `uses` [INFERRED]
- [[InvenTreeUserSetting]] - `uses` [INFERRED]
- [[LabelTemplate]] - `uses` [INFERRED]
- [[MachineConfig]] - `uses` [INFERRED]
- [[NotesImage]] - `uses` [INFERRED]
- [[NotificationEntry]] - `uses` [INFERRED]
- [[NotificationMessage]] - `uses` [INFERRED]
- [[Owner]] - `uses` [INFERRED]
- [[ParameterTemplate]] - `uses` [INFERRED]
- [[Part]] - `uses` [INFERRED]
- [[PartCategory]] - `uses` [INFERRED]
- [[PartTestTemplate]] - `uses` [INFERRED]
- [[PluginConfig]] - `uses` [INFERRED]
- [[PluginMixin_1]] - `uses` [INFERRED]
- [[PluginSetting]] - `uses` [INFERRED]
- [[ProjectCode]] - `uses` [INFERRED]
- [[ReportTemplate]] - `uses` [INFERRED]
- [[ReturnOrder]] - `uses` [INFERRED]
- [[SalesOrder]] - `uses` [INFERRED]
- [[SelectionList]] - `uses` [INFERRED]
- [[SelectionListEntry]] - `uses` [INFERRED]
- [[StockHistoryCode]] - `uses` [INFERRED]
- [[StockItem]] - `uses` [INFERRED]
- [[StockItemTestResult]] - `uses` [INFERRED]
- [[StockItemTracking]] - `uses` [INFERRED]
- [[StockLocation]] - `uses` [INFERRED]
- [[StockLocationType]] - `uses` [INFERRED]
- [[StockStatus]] - `uses` [INFERRED]
- [[SystemSetId]] - `uses` [INFERRED]
- [[Tests for the admin interface integration.]] - `rationale_for` [EXTRACTED]
- [[Tests for the admin interface integration._2]] - `rationale_for` [EXTRACTED]
- [[Tests for the admin interface integration._3]] - `rationale_for` [EXTRACTED]
- [[Tests for the admin interface integration._4]] - `rationale_for` [EXTRACTED]
- [[Tests for the admin interface integration._5]] - `rationale_for` [EXTRACTED]
- [[Tests for the admin interface integration._1]] - `rationale_for` [EXTRACTED]
- [[WebhookEndpoint]] - `uses` [INFERRED]
- [[WebhookMessage]] - `uses` [INFERRED]
- [[WebhookView]] - `uses` [INFERRED]
- [[tests.py_1]] - `contains` [EXTRACTED]
- [[tests.py_4]] - `contains` [EXTRACTED]
- [[tests.py_6]] - `contains` [EXTRACTED]
- [[tests.py_7]] - `contains` [EXTRACTED]
- [[tests.py_9]] - `contains` [EXTRACTED]
- [[tests.py_10]] - `contains` [EXTRACTED]

#graphify/code #graphify/INFERRED #community/Core_Models_&_Admin