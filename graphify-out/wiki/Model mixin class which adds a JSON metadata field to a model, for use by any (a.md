---
source_file: "inventree/src/backend/InvenTree/InvenTree/models.py"
type: "rationale"
community: "Community 2"
location: "L156"
tags:
  - graphify/rationale
  - graphify/INFERRED
  - community/Community_2
---

# Model mixin class which adds a JSON metadata field to a model, for use by any (a

## Connections
- [[.get_related_model()]] - `rationale_for` [EXTRACTED]
- [[Attachment]] - `uses` [INFERRED]
- [[DataImportStatusCode]] - `uses` [INFERRED]
- [[InvenTreeMetadata]] - `uses` [INFERRED]
- [[InvenTreeUINotifications]] - `uses` [INFERRED]
- [[MetadataMixin_1]] - `rationale_for` [EXTRACTED]
- [[NotesImage]] - `uses` [INFERRED]
- [[PluginMixinEnum]] - `uses` [INFERRED]
- [[RenderChoices_1]] - `uses` [INFERRED]

#graphify/rationale #graphify/INFERRED #community/Community_2